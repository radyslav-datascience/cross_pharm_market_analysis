...
def create_researched_drugs_coefficients(
    ...
    # Крок 3: Vectorized pivot для кожного ринку (замість вкладених циклів)
    # Pivot таблиці для SHARE_INTERNAL, INTERNAL_LIFT, EVENTS_COUNT
    pivot_share = cross_market_data.pivot_table(
        index='DRUGS_ID', columns='CLIENT_ID',
        values='SHARE_INTERNAL', aggfunc='first'
    )
    pivot_lift = cross_market_data.pivot_table(
        index='DRUGS_ID', columns='CLIENT_ID',
        values='INTERNAL_LIFT', aggfunc='first'
    )
    pivot_events = cross_market_data.pivot_table(
        index='DRUGS_ID', columns='CLIENT_ID',
        values='EVENTS_COUNT', aggfunc='first'
    )

    # Перейменовуємо колонки в pivot таблицях згідно формату та порядку заповненості
    rename_share = {mid: f'SHARE_INTERNAL_LOC_{mid}' for mid in sorted_markets if mid in pivot_share.columns}
    rename_lift = {mid: f'INTERNAL_LIFT_LOC_{mid}' for mid in sorted_markets if mid in pivot_lift.columns}
    rename_events = {mid: f'EVENTS_COUNT_LOC_{mid}' for mid in sorted_markets if mid in pivot_events.columns}

    pivot_share_renamed = pivot_share.rename(columns=rename_share)[list(rename_share.values())]
    pivot_lift_renamed = pivot_lift.rename(columns=rename_lift)[list(rename_lift.values())]
    pivot_events_renamed = pivot_events.rename(columns=rename_events)[list(rename_events.values())]

    # Один join замість 99 sequential .map() операцій — виправляє PerformanceWarning
    result = result.set_index('DRUGS_ID')
    result = result.join([pivot_share_renamed, pivot_lift_renamed, pivot_events_renamed], how='left')
