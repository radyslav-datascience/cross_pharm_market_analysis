...
# ПРИМІТКА: знижено з 5 до 4 для масового розрахунку (99 ринків) —
# більше RAM headroom при тривалій роботі (1-2 години)
MAX_WORKERS = 4
...
# Якщо жоден ринок не завершився за цей час — pipeline вважає що workers зависли
# 1800 сек = 30 хв — запас для найбільших ринків серед 99
# Після першого масового запуску відкалібрувати на основі реальних даних
MARKET_TIMEOUT_SEC = 1800
...
def get_optimal_threads() -> int:
    ...
    # Thread explosion guard: загальна кількість потоків не повинна
    # перевищувати подвоєну кількість логічних ядер
    total_threads = OPTIMAL_WORKERS * optimal
    if total_threads > CPU_LOGICAL_CORES * 2:
        optimal = max(1, CPU_LOGICAL_CORES * 2 // OPTIMAL_WORKERS)
